...
import { 
  ...
  Percent, Tag, Megaphone, Send, ChevronRight, X
} from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  return (
    <div className="flex min-h-screen bg-[#050505] text-white font-sans selection:bg-[#D4AF37] selection:text-black overflow-hidden text-left">
      ...
      <main className="flex-1 p-12 overflow-y-auto relative text-left">
        ...
        <header className="flex justify-between items-start mb-16 text-left">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="text-left">
            <div className="flex items-center gap-3 mb-3 text-left">
              ...
              <p className="text-[#D4AF37] text-[10px] font-black uppercase tracking-[0.6em] text-left">
                {activeTab === 'clients' ? 'Protocol: Personnel Management' : activeTab === 'financials' ? 'Protocol: Fiscal Intelligence' : activeTab === 'config' ? 'Protocol: Core Reconfiguration' : 'Protocol Status: Senior Admin'}
              </p>
            </div>
          ...
          <div className="flex items-center gap-8 text-left">
            <motion.div whileFocus={{ scale: 1.05 }} className="relative group text-left">
            ...
            <motion.div 
              ...
              className="w-12 h-12 bg-[#D4AF37]/10 border border-[#D4AF37]/20 rounded-full flex items-center justify-center relative cursor-pointer hover:bg-[#D4AF37]/20 transition-all shadow-xl text-left"
            >
              <Bell size={20} className="text-[#D4AF37]" />
              <motion.div 
                animate={{ scale: [1, 1.5, 1] }}
                transition={{ repeat: Infinity, duration: 2 }}
                className="absolute top-0 right-0 w-3 h-3 bg-red-500 border-2 border-[#050505] rounded-full text-left" 
              />
            </motion.div>
          </div>
        </header>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'command' && (
            <motion.div key="command" initial="initial" animate="animate" exit="exit" variants={panelVariants} className="space-y-12 text-left">
              ...
              <motion.div variants={panelVariants} className="text-left">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-left">
                ...
                <motion.div variants={panelVariants} className="flex flex-col gap-8 text-left">
                  ...
                  <div className="p-10 border border-white/5 bg-[#D4AF37]/5 backdrop-blur-md flex flex-col justify-between text-left shadow-2xl relative overflow-hidden">
                    ...
                    <h3 className="text-xs font-black tracking-[0.4em] uppercase text-[#D4AF37] mb-8 text-left text-left">Quick Operations</h3>
                    <div className="space-y-4 text-left">
                      <ActionButton label="Add New Product" onClick={() => setIsModalOpen(true)} />
                      
                      {/* --- UPDATED: Action Button rewired to Financials Tab --- */}
          ...
          {activeTab === 'broadcast' && (
            <motion.div key="broadcast" initial="initial" animate="animate" exit="exit" variants={panelVariants} className="space-y-12 text-left">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 text-left">
                <section className="space-y-6 text-left"><div className="flex items-center gap-3 text-left"><div className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-pulse text-left" /><h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-500 text-left text-left">Internal Staff Protocol</h2></div><StaffNoticeManager onSuccess={() => setArchiveRefresh(prev => prev + 1)} /></section>
                <section className="space-y-6 text-left"><div className="flex items-center gap-3 text-left"><div className="w-1.5 h-1.5 bg-[#D4AF37] rounded-full animate-pulse text-left" /><h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-[#D4AF37] text-left text-left">Customer Promotion Engine</h2></div><PromotionNoticeManager onSuccess={() => setArchiveRefresh(prev => prev + 1)} /></section>
              </div>
          ...
          {activeTab === 'promotions' && (
            <motion.div key="promotions" initial="initial" animate="animate" exit="exit" variants={panelVariants} className="space-y-12 text-left">
              <div className="grid grid-cols-1 xl:grid-cols-12 gap-12 text-left">
                ...
                <div className="xl:col-span-5 bg-white/[0.02] border border-white/5 p-10 backdrop-blur-xl text-left shadow-2xl text-left">
                   <h3 className="text-xs font-black uppercase tracking-[0.4em] text-[#D4AF37] mb-8 text-left text-left">Active Protocols</h3>
                   <ActivePromotionList 
          ...
          {activeTab === 'config' && (
            <motion.div key="config" initial="initial" animate="animate" exit="exit" variants={panelVariants} className="text-left">
              <SystemConfig />
            </motion.div>
          )}

          {activeTab === 'financials' && (
            <motion.div key="financials" initial="initial" animate="animate" exit="exit" variants={panelVariants} className="text-left">
              <Financials />
            </motion.div>
          )}
      ...
      {/* --- MODAL SYSTEM WITH DISMISSIBLE OVERLAY --- */}
      <AnimatePresence>
        {isModalOpen && (
          <>
            <motion.div 
            ...
            <motion.div 
              ...
              className="fixed right-0 top-0 h-screen w-[500px] z-[70] shadow-2xl"
            >
...
const NavItem = ({ icon, label, active = false, onClick }) => (
  <motion.button whileHover={{ x: 8 }} whileTap={{ scale: 0.95 }} onClick={onClick} className={`w-full flex items-center justify-between px-6 py-4 transition-all duration-300 relative overflow-hidden group ${active ? 'bg-[#D4AF37] text-black shadow-[0_10px_30px_rgba(212,175,55,0.2)] font-black' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}>
    <div className="flex items-center gap-5 relative z-10 text-[11px] uppercase tracking-widest text-left"><motion.span animate={active ? { rotate: [0, 10, -10, 0] } : {}} transition={{ repeat: Infinity, duration: 2 }} className="text-left">{icon}</motion.span>{label}</div>
    {active && <motion.div initial={{ x: -10 }} animate={{ x: 0 }} className="text-left"><ChevronRight size={14} className="relative z-10 text-left" /></motion.div>}
    {!active && (<motion.div className="absolute left-0 top-0 h-full w-0 bg-gradient-to-r from-[#D4AF37]/20 to-transparent group-hover:w-full transition-all text-left" initial={false} transition={{ duration: 0.4 }} />)}
  </motion.button>
);

const StatCard = ({ icon, label, val, sub, trend }) => (
  <motion.div whileHover={{ y: -10, scale: 1.02, borderColor: 'rgba(212, 175, 55, 0.4)' }} className="p-10 border border-white/5 bg-white/[0.01] backdrop-blur-sm transition-all group relative text-left shadow-xl"><div className="absolute top-8 right-8 text-[#D4AF37]/10 group-hover:text-[#D4AF37]/30 transition-all duration-500 group-hover:rotate-12 group-hover:scale-110 text-left">{React.cloneElement(icon, { size: 60 })}</div><p className="text-gray-500 text-[10px] uppercase tracking-[0.3em] font-bold mb-3 text-left text-left">{label}</p><h3 className="text-5xl font-black tracking-tighter mb-2 text-left group-hover:text-[#D4AF37] transition-colors text-left">{val}</h3><p className="text-[10px] text-gray-600 font-bold uppercase tracking-widest mb-6 text-left text-left">{sub}</p><motion.div whileHover={{ scale: 1.1 }} className="flex items-center gap-2 text-[10px] font-black text-[#D4AF37] bg-[#D4AF37]/5 w-fit px-3 py-1 border border-[#D4AF37]/10 shadow-lg text-left"><ArrowUpRight size={12} className="text-left" /> {trend} </motion.div></motion.div>
);

const ActionButton = ({ label, onClick }) => (
  <motion.button whileHover={{ scale: 1.02, x: 5 }} whileTap={{ scale: 0.98 }} onClick={onClick} className="w-full py-5 px-6 border border-white/10 hover:border-[#D4AF37] hover:bg-white/5 transition-all text-left text-[10px] font-black uppercase tracking-[0.3em] flex justify-between items-center group shadow-md text-left"><span className="group-hover:text-[#D4AF37] transition-colors text-left">{label}</span><motion.div animate={{ x: [0, 5, 0] }} transition={{ repeat: Infinity, duration: 1.5 }} className="text-left"><ArrowUpRight size={14} className="text-[#D4AF37] text-left" /></motion.div></motion.button>
);
